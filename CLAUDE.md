...
- Vanilla-ES-Module in `app/static/js/` (api, state, dates, events, colors, dom, popover, month-view, week-view, tag-picker, power-view, power-format, gestures, legend, view-memory, main), CSS in `app/static/css/`. **Kein Build-Schritt** — die Module laufen direkt im Browser.
