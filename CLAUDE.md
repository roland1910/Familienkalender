...
- `GET /feed/<token>.ics` (`app/feed.py` baut das ICS, Route in `app/feed_app.py`): abonnierbarer Kalender „Familie – Roland" für Marinas Handy (ICSx⁵/DAVx⁵). Inhalt: nur Quellen mit `display_mode=filtered`, gefiltert mit derselben Familienlogik wie die Ansichten (`filter_events`, Abendgrenze), Fenster −7/+90 Tage; Titel mit Quellen-Kürzel (`shortcode`, max. 6 Zeichen A–Z/0–9, gepflegt in der Admin-Quellenzeile) als Präfix. Stabile UIDs (Hash über source_id|uid|start), REFRESH-INTERVAL/X-PUBLISHED-TTL PT15M.
